    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setFormData({
          ...
          flat_rate: 500, // Default if not in data
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateShippingSettings(settings.id, {
        flat_rate_enabled: formData.flat_rate_enabled,
    ...
    <form onSubmit={handleSubmit} className="space-y-10">
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between py-4 px-5 bg-gray-50 rounded-xl">
          <div>
            <Label className="text-base font-medium text-gray-900">Enable Shipping</Label>
            <p className="text-sm text-gray-500 mt-1">Charge shipping on orders</p>
          </div>
          <Switch
            checked={formData.flat_rate_enabled}
            onCheckedChange={(checked) => setFormData(prev => ({ ...prev, flat_rate_enabled: checked }))}
          />
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <Label className="text-base font-medium">Flat Rate (₹)</Label>
          <Input
            type="number"
            placeholder="500"
            value={formData.flat_rate}
            onChange={(e) => setFormData(prev => ({ ...prev, flat_rate: parseFloat(e.target.value) || 0 }))}
