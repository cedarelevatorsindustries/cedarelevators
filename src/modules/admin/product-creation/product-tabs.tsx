    ...
    <Card className="border-0 shadow-sm bg-gradient-to-br from-white to-gray-50/50">
      <CardContent className="p-4">
        <div className="w-full">
          <div className="flex items-center w-full bg-orange-50 p-1 text-black rounded-md border border-orange-100/50">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => onTabChange(tab.id)}
                className={`relative flex-1 px-4 py-2 text-sm font-medium whitespace-nowrap transition-colors duration-200 ${
                  activeTab === tab.id ? 'text-white' : 'text-orange-600/70 hover:text-orange-700'
                }`}
              >
                {activeTab === tab.id && (
                  <motion.div
                    layoutId="activeProductTab"
                    className="absolute inset-0 bg-orange-600 rounded-md shadow-sm"
                    initial={false}
                    transition={{
                      type: "spring",
                      stiffness: 500,
